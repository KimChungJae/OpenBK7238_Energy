...
for s in spi tune tune7 tune8 tune9 tune10 tune11 tune12 tune13 tune14 tune15 tune16 \
	...
	tune41 tune42 tune43 tune44 tune45 tune46 tune47 tune48 tune49 tune50 tune51 tune52 \
	tune53 tune54 tune55 tune57 tune58 tune59 tune60 tune61 tune62 tune63 tune64 tune65; do
	python3 "OpenBK7238_Energy_Version1/scripts/hlw8112_bk7238_${s}.py"
done

SHA="$(git rev-parse --short=12 HEAD 2>/dev/null || echo nogit)"
APP_VER="${SHA}_hlw8112_spifix65"
echo "make APP_VERSION=${APP_VER}"
make -j"$(nproc)" APP_VERSION="${APP_VER}" APP_NAME=OpenBK7238 VARIANT=hlw8112 OpenBK7238

OUT="output/${APP_VER}"
mkdir -p artifact
cp "${OUT}/OpenBK7238_${APP_VER}.rbl" artifact/
cp "${OUT}/OpenBK7238_${APP_VER}.bin" artifact/
cp "${OUT}/OpenBK7238_QIO_${APP_VER}.bin" artifact/ 2>/dev/null || true
